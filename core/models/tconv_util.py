...
class SptConvolution(Function):
    ...
    def forward(ctx,
        ...
        c_out, c_in = w2d.shape[0], w2d.shape[1]
        w2d = w2d.view(c_out, c_in, -1)
        kernel = torch.matmul(w2d, trans).permute(2,1,0)
        if not transpose:
        ...
        ctx.for_backwards = (features, kernel, neighbor_map, neighbor_offset, w2d,
                             transpose)
        return out

    @staticmethod
    def backward(ctx, grad_out):
        features, kernel, neighbor_map, neighbor_offset, w2d, transpose = ctx.for_backwards
        ...
        return grad_features, torch.einsum('cba, dbc->ad', [w2d, grad_kernel]), None, None, None, None, None


sparseconv_op = SptConvolution.apply
...
def tconv3d(inputs,
    ...
    if kernel_size == 1 and stride == 1 and dilation == 1:
        raise NotImplementedError("Kernel size 1 not implement yet ")
    elif not transpose:
        ...
        if stride > 1:
            ...
            output_features = sparseconv_op(features, trans_m, idx_query[0],
                                            idx_query[1], sizes, w2d, transpose)
            if bias is not None:
            ...
            if kernel_map is None:
                ...
                output_features = sparseconv_op(features, trans_m,
                                                kernel_map[0], kernel_map[1],
                                                kernel_map[2],  w2d, transpose)
                if bias is not None:
                    output_features += bias
                output_tensor = SparseTensor(output_features, coords,
                                             cur_stride)
        ...
        output_features = sparseconv_op(features, trans_m, kernel_map[0],
                                        kernel_map[1], kernel_map[2], w2d,
                                        transpose)
        if bias is not None:
            output_features += bias
